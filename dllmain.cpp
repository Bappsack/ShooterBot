...
DWORD WINAPI DLLStart(LPVOID param)
{
	...
	TMiniGameManager* manager = **(TMiniGameManager***)(FindPattern((BYTE*)0x500000,
		0x200000,
		(BYTE*)("\xA1\x00\x00\x00\x00\x8B\x08\xFF\x51\x00\xA1\x00\x00\x00\x00\xE8\x00\x00\x00\x00\x33\xD2\xA1\x00\x00\x00\x00\xE8\x00\x00\x00\x00\x33\xD2\xA1\x00\x00\x00\x00\xE8\x00\x00\x00\x00\x33\xD2\xA1\x00\x00\x00\x00\xE8\x00\x00\x00\x00\x33\xD2"), "x????xxxx?x????x????xxx????x????xxx????x????xxx????x????xx") + 1);

	MiniGame_3* game = manager->miniGame_3;

	//Check if the pointer to MiniGame_3 is correct
